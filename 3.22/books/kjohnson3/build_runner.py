...
~/env/bin/python /Users/biocbuild/BBS/3.22/books/kjohnson3/build_runner.py &
...
HOSTNAME = "kjohnson3"
BIOC_VERSION = "3.22"
LOG_PATH = f"/Users/biocbuild/bbs-{BIOC_VERSION}-books/log/build_runner.log"

def build_runner(logger):
    logger.debug("START job")
    yyyymmdd = date.today().strftime('%Y%m%d')
    run_path = f"/Users/biocbuild/BBS/{BIOC_VERSION}/books/{HOSTNAME}"
    log_path = f"/Users/biocbuild/bbs-{BIOC_VERSION}-books/log/{HOSTNAME}-{yyyymmdd}-run.log"
...
if __name__ == "__main__":
    ...
    build_days = [every().monday,
                  every().wednesday,
                  every().friday]
    for d in build_days:
        d.at("6:15", timezone("US/Eastern")).do(build_runner, logger=logger)
    while True:
        run_pending()
        sleep(1)
