...
def _write_glyph_box(out, status, toggleable=False):
    if toggleable:
        ...
        checkbox_html = ''
    TRcontent  = '<TD style="width: 90px; padding-left: 6px;">%s</TD>' % \
                 _status_as_glyph(status)
    TRcontent += '<TD style="width: 22px;">%s</TD>' % checkbox_html
...
def write_explain_glyph_table(out):
    ...
    out.write('<TABLE style="width: 680px; border: solid black 1px; border-collapse: collapse;">\n')
    out.write('<TR>\n')
...
def write_gcard_list(out, allpkgs, leafreport_ref=None):
    ...
    out.write('<TABLE id="THE_GCARD_LIST" class="gcard_list">\n')
    nb_pkgs = len(allpkgs)
    if full_list:
...
def write_compact_gcard_list(out, node, allpkgs):
    nb_pkgs = len(allpkgs)
    out.write('<TABLE id="THE_GCARD_LIST" class="compact gcard_list">\n')
