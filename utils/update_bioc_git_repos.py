...
def update_packages(pkgsrctree, pkgs, pkg_git_branch=None, skip=None):
    ...
    for pkg in pkgs:
        ...
        pkg_git_repo_url = 'https://%s/packages/%s' % (gitserver, pkg)
...
def update_manifest(manifest_git_branch=None):
    ...
    print()
    bbs.gitutils.clone_or_pull_repo(manifest_git_clone, manifest_git_repo_url,
                                    manifest_git_branch, cleanup=True)
    return
