...
For example, for the BioC 3.11 release, we will need to do this for all
the packages listed in the `software.txt`, `data-experiment.txt`, and
`workflows.txt` files of the `RELEASE_3_11` branch of the `manifest`
repo.
...
before the BioC 3.11 builds start for software, workflows, and data-experiment
packages.

Look for the prerun jobs in the crontab for the `biocbuild` user on the main
BioC 3.11 builder to get the times the software and data-experiment builds get
...
* Update this document to reflect the BioC version to be released i.e.
  replace all occurrences of `3.11` and `RELEASE_3_11` with appropriate
...
* Populate `git.bioconductor.org` with git clones of the `manifest` repo
  ...
  in sections **C.**, **D.**, and **E.** below on the day prior to the release.
      ...
      # clone `manifest` repo
      $BBS_HOME/utils/update_bioc_git_repos.py manifest RELEASE_3_11

      # clone software package repos (takes approx. 1h20)
      time $BBS_HOME/utils/update_bioc_git_repos.py software master RELEASE_3_11

      # clone data-experiment package repos (takes approx. 1h50)
      time $BBS_HOME/utils/update_bioc_git_repos.py data-experiment master RELEASE_3_11

      # clone workflow package repos (takes approx. 4 min)
      time $BBS_HOME/utils/update_bioc_git_repos.py workflows master RELEASE_3_11
...
The `RELEASE_3_10` lines in `gitolite-admin/conf/packages.conf` were commented
...
NOTE: Do not change the branch from RELEASE_3_10 to RELEASE_3_11, it is
not a good solution. Maintainers now will be able to push their own
RELEASE_3_11 branch before we are able to create it at release
...
### C5. Checkout/update the `RELEASE_3_11` branch of the `manifest` repo
...
be the file from the `RELEASE_3_11` branch of the `manifest` repo:
...
of applying and pushing the changes described in **A. Introduction**.
    ...
    $BBS_HOME/utils/bump_version_and_create_branch.py --push RELEASE_3_11 $pkgs_in_manifest >bump_version_and_create_branch.log 2>&1 &
...
a 2-pass run:
    ...
    $BBS_HOME/utils/bump_version_and_create_branch.py RELEASE_3_11 $pkgs_in_manifest >bump_version_and_create_branch.log1 2>&1 &
    # Second pass (push all the changes):
    $BBS_HOME/utils/bump_version_and_create_branch.py --push RELEASE_3_11 $pkgs_in_manifest >bump_version_and_create_branch.log2 2>&1 &
...
* In the 2-pass run, the second pass checks the packages and applies the
  ...
  `RELEASE_3_11` branch) before pushing the changes.
...
### F1. Enable push access to new `RELEASE_3_11` branch
...
- If not done already, replace all instances of `RELEASE_3_10` with
  `RELEASE_3_11`.

- Uncomment all `RELEASE_3_11` and `master` lines.
...
### F3. Switch `BBS_BIOC_GIT_BRANCH` from `master` to `RELEASE_3_11` on main BioC 3.11 builder

DON'T FORGET THIS STEP! Its purpose is to make the BioC 3.11 builds grab the
`RELEASE_3_11` branch of all packages instead of their `master` branch.

Login to the main BioC 3.11 builder as `biocbuild` and replace

    export BBS_BIOC_GIT_BRANCH="master"

with

    export BBS_BIOC_GIT_BRANCH="RELEASE_3_11"

in `~/BBS/3.11/config.sh`
...
in `~/BBS/3.11/config.bat`

Then remove the `manifest` and `MEAT0` folders from `~/bbs-3.11-bioc/`,
`~/bbs-3.11-data-experiment/`, and `~/bbs-3.11-workflows/`. They'll get
...
to edit the release version in the function.
	...
	clone_and_push_git_repo(package, release="RELEASE_3_11")

	
This function will push the `RELEASE_3_11` branch to github and sync
