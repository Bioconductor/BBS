...
* 1st version bump: bump x.y.z version to even y in the master branch
* Branch creation: Create the release branch
* 2nd version bump: Bump x.y.z version to odd y in the **master** branch
...
data-experiment builds before the BioC 3.6 release will start on
Mon Oct 30 at 17:00 EST and on Tue Oct 31 at 9:35 am EST, respectively.
...
and populate it with git clones of the manifest and all packages repos:
...
before the last software builds preceding the official release get kicked
off (see Introduction). The full procedure should take less than 2 hours.
...
### 3. Update the manifest git clone and checkout the `RELEASE_3_6` branch
...
Steps 5-12 must be performed from this folder
...
### 10. 2nd version bump (to odd y)

    ~/BBS/utils/bump_pkg_versions.sh test odd
    ~/BBS/utils/bump_pkg_versions.sh odd

### 11. Commit 2nd version bump

Same as step 8. EXCEPT that commit message now is:
