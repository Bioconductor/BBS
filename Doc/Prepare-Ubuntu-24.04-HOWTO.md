# How to set up an Ubuntu 24.04 system for the daily builds
...
Note that with the above setting (`LANG="en_US.UTF-8"`), and if `LC_TIME`
is not defined in the `/etc/default/locale` file, the `date` command prints
the time in AM/PM format on Ubuntu 24.04:

    date
    # Tue Apr 20 04:25:04 PM EDT 2021

To change this to 24-hour format (a.k.a. military time), which is highly
recommended, set `LC_TIME` to `en_GB` like this:
...
Finally set the timezone to NY:

    sudo timedatectl set-timezone America/New_York

and check the time again with `date`.
...
under `BBS/Ubuntu-files/24.04/`. They can be installed with:

    sudo apt-get install <pkg1> <pkg2> <pkg3> ...

They can also be installed with the following code where `BBS_UBUNTU_PATH`
is the path to `BBS/Ubuntu-files/24.04` and `BBS_PACKAGES_FILE` is the
...
[apt_nice_to_have.txt](../Ubuntu-files/24.04/apt_nice_to_have.txt)

#### Packages required by the build system itself (BBS)

[apt_required_build.txt](../Ubuntu-files/24.04/apt_required_build.txt)

#### Packages for compiling R

[apt_required_compile_R.txt](../Ubuntu-files/24.04/apt_required_compile_R.txt) are
required to compile R.

[apt_optional_compile_R.txt](../Ubuntu-files/24.04/apt_optional_compile_R.txt)
are optional;  however, some capabilities will be missing.

#### Packages needed to build vignettes and reference manuals

[apt_vignettes_reference_manuals.txt](../Ubuntu-files/24.04/apt_vignettes_reference_manuals.txt)
...
[apt_extra_fonts.txt](../Ubuntu-files/24.04/apt_extra_fonts.txt)

Note that a reboot is required to make the fix effective.

#### Packages needed by some CRAN and/or BioC packages

For CRAN packages, install [apt_cran.txt](../Ubuntu-files/24.04/apt_cran.txt).
...
4. It keeps the setup of the machine simple, easy to document, and easy
   ...
   to `BBS/Ubuntu-files/24.04/apt_bioc.txt` or other appropriate file
   under `BBS/Ubuntu-files/24.04/`. This makes it easy to automate
...
doesn't seem to be here anymore in Ubuntu 24.04.
...
For more about installing .NET, see https://docs.microsoft.com/en-us/dotnet/core/install/linux-ubuntu.
...
1. Make a copy of the current report for comparison.
2. From the biocpush account on the primary builders, rename repository to keep
...
3. Recreate repository with the appropriate CRAN structure and PACKAGE files.
See https://github.com/Bioconductor/BBS/blob/devel/Doc/Set-up-propagation-HOWTO.md#create-fake-xy-repositories.
4. Run or rerun postrun.
5. Review report. If you see a lot of red LEDs (more than the previous report),
replace the repository with the old repository made in an earlier step then
rerun postrun. Try flushing repo after issue causing red LEDs is resolved.
...
Watch that the script is running as expected.
