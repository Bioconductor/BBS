...
{
    print("in install.packages.both()")
    print("pkgs is:")
    print(pkgs)
    print("repos:")
    print(repos)
                              ...
                              multiArch=FALSE)
{
    ...
    INSTALL_opts = ""
    if ((type=="source") && multiArch)
        INSTALL_opts="--merge-multiarch"
    
    if((type == "source") && grepl("^x86_64-apple-darwin", R.Version()$platform))
...
preinstallPkgs <- function()
{
    pkgs <- c("ada", "Cairo", "cairoDevice",
              "fields", "gmp", "igraph", "kza",
              "mclust", "ncdf",
              "Rcpp", "RCurl", "rgl",
              "rJava", "RJSONIO", "Rlibstree", "RODBC",
              "SSOAP", "tcltk2", "XML", "XMLRPC")
    ...
    # at some point rggobi may be available for mac again. Until then,
    # install as binary on windows only (source build will likely fail).
    if(.Platform$OS.type == "windows")
        pkgs <- c(pkgs, "gsl", "h5r", "ks", "nloptr",
            "rbamtools", "rggobi", "RGtk2",  "Rmpi", "tkrplot")

    if (Sys.info()["sysname"] == "Darwin")
        pkgs <- c(pkgs, "biganalytics", "RPostgreSQL", "RMySQL")

    install.packages2(pkgs, repos=dep_repos)

    # check to make sure they were all installed.
    # This just checks that they are in installed.packages(); we may want
    # to get more aggressive and actually try and load them.
    ip <- rownames(installed.packages())
    failed <- pkgs[!pkgs %in% ip]
    if (length(failed))
    {
        for (i in 1:3)
        {
                if (length(failed))
                {
                    message("ERROR! Sanity check failed! not all packages present")
                    message("after preinstallPkgs()! These are not installed:")
                    print(failed)
                    print(paste("Retrying, attempt", i))
                    install.packages2(failed, repos=dep_repos)
                    ip <- rownames(installed.packages())
                    failed <- pkgs[!pkgs %in% ip]
                } else {
                    message("We are ok now.")
                    return()
                }
        }
        # if we get here, we failed 3 times.
        message("Failed three times. Builds may have errors!")
        ### q("no", status=1111L)
    } else {
        message("Sanity check succeeded! All packages were actually installed.")
    }
