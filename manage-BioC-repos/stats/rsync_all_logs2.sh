...
echo ""
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo "> RETRIEVING APACHE2 LOG FILES FROM $LOGS_REMOTE_PATH/master"
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo ""
rsync -ave ssh "$LOGS_REMOTE_PATH/master/bioconductor-access.log-*" $LOGS_LOCAL_PATH/apache2/master
