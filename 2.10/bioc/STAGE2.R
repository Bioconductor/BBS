                              ...
                              multiArch=FALSE)
{
    ...
    if(grepl("^x86_64-apple-darwin", R.Version()$platform)) multiArch = FALSE
...
preinstallPkgs <- function()
{
    pkgs <- c("ada", "Cairo", "cairoDevice",
              "fields", "gtkDevice", "igraph", "mclust", "ncdf",
...
installExtraArchs <- function(pkg)
{
    main_arch <- .Platform$r_arch
    archs <- list.files(paste(Sys.getenv("R_HOME"),"etc",sep="/"))
    remove <- c(main_arch, "repositories")
    extra_archs <- list()
    for (item in archs) if (!item %in% remove) extra_archs <- c(extra_archs, item)
    r_cmd = paste(Sys.getenv("R_HOME"), "bin", "R", sep="/")
    for (arch in extra_archs) {
        cat(paste("STAGE2.R>Installing", pkg, "for architecture", arch, "....\n"))
        full_r_cmd <- paste(r_cmd, "--arch", arch, "--slave")
        cmd <- paste("echo \"source('", Sys.getenv("BBS_STAGE2_R_SCRIPT"),"');",
          "installExtraArchs2('", pkg,"')\"", sep="")
        
        cmdline <- paste(cmd, "|", full_r_cmd)
        system(cmdline)
    }
}

installExtraArchs2 <- function(pkg) 
{
    repos <- c(target_repo, dep_repos)
    type="source"
    install.packages(pkg, repos=repos, type=type,
        INSTALL_opts="--libs-only --no-multiarch")
}
